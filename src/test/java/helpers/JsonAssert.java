        ...
        while (attributes.hasNext()) {
            ...
            if (expectedJSON.has(attribute) && actualJSON.has(attribute)) {
                ...
                if (expectedValue.equals("IGNORE") || actualValue.equals("IGNORE")) {
                    ...
                    System.out.println("ERROR> el attribute: [" + attribute + "] > actual: " + actualValue + " vs expected: " + expectedValue);
                    areEqual = false;
                    msgBuilder.append("\nERROR> el attribute: [").append(attribute).append("] > actual: ").append(actualValue).append(" vs expected: ").append(expectedValue);
                }
            } else {
                System.out.println("ERROR> el attribute no existe: [" + attribute + "]");
                msgBuilder.append("\nERROR> el attribute no existe: [").append(attribute).append("]");
                areEqual = false;
            }
        }
